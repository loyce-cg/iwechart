...
import { CustomSelectView } from "../component/customSelect/CustomSelectView";
...
Web.Starter.addEventListener<Web.Types.event.InstanceRegisteredEvent<Web.window.sectionsummary.SectionSummaryWindowView>>("instanceregistered", event => {
    if (event.instance && event.instance.className == "com.privmx.core.window.sectionsummary.SectionSummaryWindowView") {
        let kvdb = new TaskGroupsPanelView(event.instance, event.instance.personsComponent);
...
Web.Starter.addEventListener("request-custom-select-view", event => {
    let view = new CustomSelectView((<any>event).parent);
    (<any>event).parent.registerCustomSelectView((<any>event).name, view);
}, "tasks", "ethernal");
